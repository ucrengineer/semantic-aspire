...
app.MapGet("/summarizeConversation", async (CancellationToken cancellationToken) =>
{
    kernel.Plugins.TryGetPlugin(nameof(ConversationSummaryPlugin), out var conversationSummaryPlugin);

    var summary = await kernel.InvokeAsync(
        conversationSummaryPlugin["SummarizeConversation"],
        new() { ["input"] = GetChatTranscript() },
        cancellationToken);

    return summary.GetValue<string>();

})
    .WithDescription("Summarizes a conversation")
